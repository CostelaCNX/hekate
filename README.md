...
| emupath={FOLDER path}  | Forces emuMMC to use the selected one. (=emuMMC/RAW1, =emuMMC/SD00, etc). emuMMC must be created by hekate because it uses the raw_based/file_based files. Not needed when using `sdroot`, since the pack's own `emuMMC/` is swapped in automatically. |
...
| Config option             | Description                                                |
| ------------------------- | ---------------------------------------------------------- |
| sdroot={FOLDER path}      | Makes the specified folder the logical SD root before booting (e.g. `sdroot=SD2`): everything found inside it, including `atmosphere/`, `switch/` and its own `emuMMC/`, is swapped with its counterpart at the physical SD root. Enables a fully separate Atmosphere/HOS/emuMMC environment on the same SD card, with no need for `emupath`. The swap is automatically reversed on the next boot. The folder name is free (`SD2`, `SD3`, `old_fw`, etc). No limit on number of environments. Only `bootloader/`, `payload.bin`, `boot.ini` and `boot.dat` stay on the physical root and are never swapped — they belong to hekate itself, not to a pack. Any other key on the same boot entry (`fss0`, `kip1`, etc.) is resolved *after* the swap, so use paths as if the pack's folder were already the SD root (e.g. `fss0=atmosphere/package3`, not `fss0=SD2/atmosphere/package3`). Keys that point at hekate's own infrastructure (`icon`, `logopath`, `payload`) still need the physical path, since `bootloader/` inside the pack is not swapped. |
